...
G4THitsMap<G4double>* 
WLGDEventAction::GetHitsCollection(G4int hcID,
...
WLGDEventAction::GetVecHitsCollection(G4int hcID,
  				      const G4Event* event) const
{
  auto hitsCollection 
  ...
  if ( ! hitsCollection ) {
    G4ExceptionDescription msg;
    msg << "Cannot access hitsCollection ID " << hcID; 
    G4Exception("WLGDEventAction::GetVecHitsCollection()",
      "MyCode0003", FatalException, msg);
  }         
...
{  
  ...
  if ( fCollID_lar < 0 ) {
    fCollID_water 
      = G4SDManager::GetSDMpointer()->GetCollectionID("WaterDet/Edep");
    fCollID_lar 
      = G4SDManager::GetSDMpointer()->GetCollectionID("LarDet/Edep");
    fCollID_ular 
      = G4SDManager::GetSDMpointer()->GetCollectionID("ULarDet/Edep");
    fCollID_ge 
      = G4SDManager::GetSDMpointer()->GetCollectionID("GeDet/Edep");

    fLocID_water 
      = G4SDManager::GetSDMpointer()->GetCollectionID("WaterDet/Loc");
    fLocID_lar 
      = G4SDManager::GetSDMpointer()->GetCollectionID("LarDet/Loc");
    fLocID_ular 
      = G4SDManager::GetSDMpointer()->GetCollectionID("ULarDet/Loc");
    fLocID_ge 
      = G4SDManager::GetSDMpointer()->GetCollectionID("GeDet/Loc");
  }
  ...
  // 8 columns to fill
  if (!edep_lar.empty()) { // clear vectors
    edep_water.clear();
    edep_lar.clear();
    edep_ular.clear();
    edep_ge.clear();
