...
void B4dEventAction::PrintEventStatistics(G4double absoEdep, G4double gapEdep) const
{
  // Print event statistics
  //
  G4cout << "   Absorber: total energy: " << std::setw(7)
         << G4BestUnit(absoEdep, "Energy") << G4endl
         << "        Gap: total energy: " << std::setw(7) << G4BestUnit(gapEdep, "Energy")
         << G4endl;
}
...
void B4dEventAction::EndOfEventAction(const G4Event* event)
{
  ...
  if(fAbsoEdepHCID == -1)
  {
    fAbsoEdepHCID = G4SDManager::GetSDMpointer()->GetCollectionID("Calor_abs/eDep");
    fGapEdepHCID  = G4SDManager::GetSDMpointer()->GetCollectionID("Calor_gap/eDep");
  }
  ...
  if((printModulo > 0) && (eventID % printModulo == 0))
  {
    ...
    PrintEventStatistics(absoEdep, gapEdep);
  }
}
