...
GB03PSLocation::GB03PSLocation(G4String name, const G4String& unit, G4int depth)
: G4VPrimitiveScorer(name, depth)
, HCID(-1)
, fcounter(0)
, EvtMap(0)
...
G4bool GB03PSLocation::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  //  if ( aStep->GetPostStepPoint()->GetStepStatus() == fGeomBoundary ) return FALSE; //
  //  collisions
...
void GB03PSLocation::SetUnit(const G4String& unit) { CheckAndSetUnit(unit, "Length"); }
