...
GB03DetectorConstruction::~GB03DetectorConstruction() { delete fDetectorMessenger; }
...
void GB03DetectorConstruction::DefineMaterials()
{
  ...
  new G4Material(name = "Aluminium", z = 13., a = 26.98 * g / mole,
                 density = 2.700 * g / cm3);
  new G4Material(name = "Silicon", z = 14., a = 28.09 * g / mole,
                 density = 2.33 * g / cm3);
  new G4Material(name = "Iron", z = 26., a = 55.85 * g / mole, density = 7.87 * g / cm3);
  new G4Material(name = "ArgonGas", z = 18., a = 39.95 * g / mole,
...
void GB03DetectorConstruction::SetupGeometry()
{
  ...
  fWorldPhysical =
    new G4PVPlacement(0, G4ThreeVector(), fWorldLogical, "World", 0, false, 0);
  ...
  fCalorPhysical       = new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), fCalorLogical,
                                     fCalName, fWorldLogical, false, 0);
  ...
  fLayerPhysical = new G4PVReplica(fCalName + "_Layer", fLayerLogical, fCalorLogical,
  ...
  fGapPhysical =
    new G4PVPlacement(0, G4ThreeVector(0., 0., fLayerThickness / 4.), fGapLogical,
...
void GB03DetectorConstruction::SetupDetectors()
{
  ...
  for(G4int j = 0; j < 2; j++)
  {
    ...
    // to its mother.
    G4PSTermination* primitive;
    primitive =
      new G4PSTermination("PopNW", j);  // no unit, terminated tracks inside the cell
    det->RegisterPrimitive(primitive);
    primitive =
      new G4PSTermination("PopWW", j);  // no unit, terminated tracks inside the cell
    primitive->Weighted(true);          // weighted value
    det->RegisterPrimitive(primitive);

    GB03PSLocation* locprimitive;
    locprimitive = new GB03PSLocation("Loc", j);  // unit [m]
    det->RegisterPrimitive(locprimitive);
...
{
  G4cout << "--------------------------------------------------------" << G4endl;
  G4cout << " Absorber is made of " << fAbsorberMaterial->GetName() << G4endl
