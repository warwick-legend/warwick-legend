...
void GB03DetectorConstruction::SetupDetectors()
{
  ...
  for(G4int j=0;j<2;j++)
  {
    ...
    primitive = new G4PSTermination("PopNW",j); // no unit, terminated tracks inside the cell
    det->RegisterPrimitive(primitive);
    primitive = new G4PSTermination("PopWW",j); // no unit, terminated tracks inside the cell
