...
void StandaloneVisAction::Draw()
{
  ...
  if(pVisManager)
  {
    ...
    // volumes
    G4Box world("World", worldside * cm, worldside * cm,
                worldside * cm);  // larger than rest
    pVisManager->Draw(world, G4VisAttributes(G4Colour::Gray()));

    G4Box cavernbox("Cavern", (hallhside + stone) * cm, (hallhside + stone) * cm,
                    (hallhside + stone) * cm);  // shift to world top
    pVisManager->Draw(cavernbox, G4VisAttributes(G4Colour::White()),
                      G4Translate3D(0. * cm, 0. * cm, offset * cm));

    G4Box hallbox("Hall", hallhside * cm, hallhside * cm, hallhside * cm);  // Hall volume
    pVisManager->Draw(hallbox, G4VisAttributes(G4Colour::Cyan()),
                      G4Translate3D(0. * cm, 0. * cm, offset * cm));

    G4Box tankbox("Cout", tankhside * cm, tankhside * cm,
                  tankhside * cm);  // Cryostat tank wall
    pVisManager->Draw(tankbox, G4VisAttributes(G4Colour::Grey()));

    G4Box waterbox("insulator", (tankhside - outerwall) * cm,
                   (tankhside - outerwall) * cm,
                   (tankhside - outerwall) * cm);  // inside outer wall
    pVisManager->Draw(waterbox, G4VisAttributes(G4Colour::Blue()));

    G4Box cryoouterbox("membrane", (tankhside - outerwall - insulation) * cm,
                       (tankhside - outerwall - insulation) * cm,
                       (tankhside - outerwall - insulation) * cm);  // inside insulation
    pVisManager->Draw(cryoouterbox, G4VisAttributes(G4Colour::Green()));

    G4Box larbox("LAr", larside * cm, larside * cm,
                 larside * cm);  // liquid argon volume inside membrane
