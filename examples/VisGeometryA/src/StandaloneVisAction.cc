...
void StandaloneVisAction::Draw() {
  ...
  // volumes
    G4Tubs world("World", 0.0*cm, (hallrad+stone+0.1)*cm, (hallhheight+stone+offset+0.1)*cm, 0.0, CLHEP::twopi); // larger than rest
    pVisManager->Draw(world, G4VisAttributes(G4Colour::Gray()));

    G4Tubs caverntub("Cavern", 0.0*cm, (hallrad+stone)*cm, (hallhheight+stone)*cm, 0.0, CLHEP::twopi); // shift to world top
    pVisManager->Draw(caverntub, G4VisAttributes(G4Colour::White()), G4Translate3D(0.*cm, 0.*cm, offset*cm));

    G4Tubs halltub("Hall", 0.0*cm, hallrad*cm, hallhheight*cm, 0.0, CLHEP::twopi); // Hall volume
    pVisManager->Draw(halltub, G4VisAttributes(G4Colour::Cyan()), G4Translate3D(0.*cm, 0.*cm, offset*cm));

    G4Cons tankcone("Tank", 0.0*cm, (tankrad+tankwallbot)*cm, 0.0*cm, (tankrad+tankwalltop)*cm, tankhheight*cm, 0.0, CLHEP::twopi); // Water tank wall
    pVisManager->Draw(tankcone, G4VisAttributes(G4Colour::Grey()));

    G4Tubs watertub("Water", 0.0*cm, tankrad*cm, (tankhheight-tankwallbot)*cm, 0.0, CLHEP::twopi); // Water tank volume
    pVisManager->Draw(watertub, G4VisAttributes(G4Colour::Blue()));

    G4Tubs cryooutertub("Cout", 0.0*cm, cryrad*cm, cryhheight*cm, 0.0, CLHEP::twopi);  // cryostat outer volume
    pVisManager->Draw(cryooutertub, G4VisAttributes(G4Colour::Green()));

    G4Tubs cryovactub("Cvac", 0.0*cm, (cryrad-cryowall)*cm, cryhheight*cm, 0.0, CLHEP::twopi);   // cryostat vacuum gap volume
    pVisManager->Draw(cryovactub, G4VisAttributes(G4Colour::Grey()));

    G4Tubs cryoinnertub("Cinn", 0.0*cm, (cryrad-cryowall-vacgap)*cm, cryhheight*cm, 0.0, CLHEP::twopi); // cryostat inner volume
    pVisManager->Draw(cryoinnertub, G4VisAttributes(G4Colour::Green()));

    G4Tubs lartub("Lar", 0.0*cm, (cryrad-2*cryowall-vacgap)*cm, cryhheight*cm, 0.0, CLHEP::twopi); // liquid argon volume
    pVisManager->Draw(lartub, G4VisAttributes(G4Colour::White()));

    G4Tubs lidtub("Lid", 0.0*cm, cryrad*cm, cryowall/2.0*cm, 0.0, CLHEP::twopi); // close cryostat cylinder at top and bottom
    pVisManager->Draw(lidtub, G4VisAttributes(G4Colour::Grey()), G4Translate3D(0.*cm, 0.*cm, (cryhheight+cryowall/2.0)*cm));
    pVisManager->Draw(lidtub, G4VisAttributes(G4Colour::Grey()), G4Translate3D(0.*cm, 0.*cm, -(cryhheight+cryowall/2.0)*cm));
